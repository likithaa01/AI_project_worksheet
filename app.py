...
def main():
    ...
    with tabs[4]:
        st.header("AI Image Search")
        search_mode = st.radio("Choose Search Type", ["Generate Caption", "Textual Search", "Visual Search"])

        if search_mode == "Generate Caption":
            ...
            if uploaded_img:
                ...
                if st.button("🧠 Generate Caption", type="primary"):
                    try:
                        from services.caption import generate_caption  # Ensure you have this import in the correct file

                        image_bytes = uploaded_img.read()
                        mime_type = uploaded_img.type or "image/jpeg"
                        file_name = uploaded_img.name or "uploaded.jpg"
                        ...
                        caption = result.get("caption") or result.get("result") or "No caption returned."
        ...
        elif search_mode == "Textual Search":
            query = st.text_input("Enter your text query")
            if query and st.button("Search Images"):
                from services.textual_search import search_by_text
                results = search_by_text(api_key=st.session_state.api_key, text=query)
                for img in results.get("results", []):
                    st.image(img.get("url"))
            elif search_mode == "Visual Search":
                ref_img = st.file_uploader("Upload reference image", type=["jpg", "png"], key="visualsearch")
                if ref_img and st.button("Search Similar"):
                    from services.visual_search import search_by_image
                    results = search_by_image(api_key=st.session_state.api_key, image_bytes=ref_img.read())
                    for img in results.get("results", []):
                        st.image(img.get("url"))
